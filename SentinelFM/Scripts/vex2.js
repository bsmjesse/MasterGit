...
function AddControlBox(){
    try {
        ...
        el.style.left = "-10px";
        //        if (maptype == "GeoZone" || maptype == "Landmark" || maptype == "GeoZone_Landmarks") 
        //            el1.style.top = "297px";
        //        else 
        el.style.top = "297px";
...
function CollapseToolbar(){
    if (controlBoxExpanded && controlBox != null) {
        ...
        map.HideControl(controlBox);
        //        if (maptype=="Map"||maptype=="History")
        //            map.HideControl(drawcontrol);
    }
    else {
        controlBoxExpanded = true;
        map.ShowControl(controlBox);
        //        if (maptype=="Map"||maptype=="History")
                        ...
                        case "Geozone Layer":
                            ...
                            for (var i = 0; i < xlm.length; i++) {
                                try {
                                    ...
                                    if (lminfo[1] == name) {
                                        ViewGeozone(lminfo, true);
                                        ViewGeozone(lminfo, false);
                                        return;
                                    }
                                } 
                                catch (err) {
                            ...
                            for (var i = 0; i < xlm.length; i++) {
                                try {
                                    ...
                                    if (lminfo[1] == name) {
                                        ...
                                        ViewRadius(point, radius, true);
                                        ViewRadius(point, radius, false);
...
function ViewGeozone(geoinfo, setsize){
    //alert("ViewGeozone");
...
function FrameResize(owidth, oheight){
	 //obj, widthOffset, heightOffset){
	w = document.body.clientWidth+owidth;
	h = document.body.clientHeight+oheight;
    //alert(w+ "\r\n" + h);
    document.getElementById("vex").style.width = w + "px";
    document.getElementById("vex").style.height = h + "px";
    if (map != null) 
        map.Resize();
