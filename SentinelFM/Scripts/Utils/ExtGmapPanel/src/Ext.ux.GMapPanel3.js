...
markers: [{
    ...
    getStreetViewMap : function (ourmap,newPos)
    {
        var sv = new google.maps.StreetViewService();
        sv.getPanoramaByLocation(newPos, 50, function(data, status) {
        if (status == google.maps.StreetViewStatus.OK) {
            //var panorama=ourmap.getStreetview();
            //console.log("Panorama: " + panorama);
             ourmap.setPano(data.location.pano);
             ourmap.setPov({
                heading: 270,
                pitch: 0,
                zoom: 1
              });
              ourmap.setVisible(true);
    ...
    apiReady : function(){
        ...
        if (this.rendered){
          ...
          if (this.gmapType === 'panorama'){
              ...
              if(this.setCenter)
              {
               var newPos = new google.maps.LatLng(this.setCenter.lat,this.setCenter.long);               
               this.gmap = new google.maps.StreetViewPanorama(this.body.dom);
               this.getStreetViewMap(this.gmap,newPos);  
              }
          }
          ...
          if (typeof this.setCenter === 'object') {
              if (typeof this.setCenter.geoCodeAddr === 'string'){
                  ...
                  if (typeof this.setCenter.marker === 'object' && typeof point === 'object') {
                        if(this.setCenter.marker.listeners)
                        {
                        this.addMarker(point, this.setCenter.marker, this.setCenter.marker.clear,true,this.setCenter.marker.listeners); //Recenter map to marker position
                        }
                        else
                        {
