var browserTypes = { "unknown": "??", "internetExplorer": "ie", "firefox": "ff", "safari": "sf", "googleChrome": "gc", "opera": "op" };
var agentParams = { "browserType": 0, "version": 1 };
var agent = new Array(browserTypes.unknown, 0);
var isIE6 = false;
var BsmVE_Version = "v1.1.3";

function BsmVE(type) {


    var avldesc = '<table cellpadding="0" cellspacing="0" border="0"><tr><td class="caption">Timestamp:</td><td class="value">##TIMESTAMP##</td></tr><tr><td class="caption">Address:</td><td class="value">##ADDRESS##</td></tr><tr><td class="caption">Speed:</td><td class="value">##SPEED##</td></tr><tr><td class="caption">Status:</td><td class="value">##STATUS##</td></tr><tr><td>&nbsp;</td><td style="text-align:right;"></td></tr></table>';
    var avlhdesc = '<table cellpadding="0" cellspacing="0" border="0"><tr><td class="caption">Timestamp:</td><td class="value">##TIMESTAMP##</td></tr><tr><td class="caption">Address:</td><td class="value">##ADDRESS##</td></tr><tr><td class="caption">Speed:</td><td class="value">##SPEED##</td></tr><tr><td class="caption">Status:</td><td class="value">##STATUS##</td></tr><tr><td>&nbsp;</td><td style="text-align:right;"></td></tr><tr><td class="caption">Details:</td><td class="value">##DETAILS##</td></tr><tr><td>&nbsp;</td><td style="text-align:right;"></td></tr></table>';

    //VE control
    this.map = null;
    this.maptype = type;
    this.geowidthtrigger = 20.0; //0.5;	 --> 20.0 == 200km
    this.landwidthtrigger = 40.0; //0.8; --> 40.0 == 100km
    this.lonwidth = 1000; //1000;
    ...
    function loadAvlDesc() {
        isIE6 = (agent[agentParams.browserType] === browserTypes.internetExplorer && agent[agentParams.version] === "6");
    };
    ...
    this.initializeVE = function() {
        try {
            ...
            if (this.maptype == "Map" || this.maptype == "History") {
            ...
            for (var i = 0; i < avlinfos.length - 1; i++) {
                ...
                try {
                    ...
                    if (avlinfo.length == 11) {
                        ...
                        var avld = avldesc;
			if (this.maptype=="History") avld = avlhdesc;
                        else if (this.maptype == "Map") {
                            cicon.ForeColor = new VEColor(0, 0, 0, 1.0);
		            cicon.TextBold = true;
		            cicon.TextItalics = true;
			    cicon.TextUnderline = true;
...
if (this.maptype=="History")avld = avld.replace("##DETAILS##", avlinfo[6]);
			
			var imsrc = "";

                        if (isIE6)
                            imsrc = "<div style=\"filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + this.imgdir + "/" + avlinfo[9] + "', sizingMethod='scale');\" ></div>";
                        else
                            imsrc = "" + this.imgdir + "/" + avlinfo[9]; //+".gif";

                        imsrc = imsrc.replace(".gif", ".ico");
                        imsrc = imsrc.replace("Idle", "IdleCircle");
            ...
            try {
                this.fleetlayer.AddShape(avlShapes);
            }
            catch (e) {
                alert("addAvlShapes error adding shape to fleetlayer:" + e.description + "\r\n" + pointline.length);
            }
...
//alert(srcs);	
    ...
    this.setLandmarkData = function(value, persist) {
        if (value == null || value == "")
            return;
        if (persist) {
            //this.eraseCookie("landmarks");
            this.createCookie("landmarks", value, false);
        }
        this.landlayer.Show();
        ...
        try {
            ...
            for (var i = 0; i < xlm.length; i++) {
                try {
                    ...
                    var pin = new VEShape(VEShapeType.Pushpin, point);
                    //var cicon = new VECustomIconSpecification();                   
                    var pinhtml = "<div style='padding-top:6px;padding-left:6px;'><img src='" + this.imgdir + "/" + lminfo[5].replace(".gif", ".ico") + "' /></div>"
            ...
            this.landlayer.AddShape(lmShapes);
            //                if (setview) 
    ...
    this.setGeoZoneData = function(value, persist) {
        if (value == null || value == "")
            return;
        if (persist) {
            //this.eraseCookie("geozones");
            this.createCookie("geozones", value, false);
        }
        this.geolayer.Show();
    ...
    this.addGeoZoneShapes = function(value) {
        try {
            ...
            var geodesc = '<table cellpadding="0" cellspacing="0" border="0"><tr><td class="caption">GeoZone Type:</td><td class="value">##TYPE##</td></tr><tr><td class="caption">Severity:</td><td class="value">##SEVERITY##</td></tr><tr><td class="caption">Points:</td><td class="value">##POINTS##</td></tr><tr><td>&nbsp;</td><td style="text-align:right;"></td></tr></table>';
            for (var i = 0; i < xlg.length; i++) {
                try {
                    ...
                    if (geoinfo[index] == 2)
                        ...
                        type = "_out";
                    var cicon = new VECustomIconSpecification();
                    cicon.Image = "" + this.imgdir + "/geozone" + type + ".gif";
                    pin.SetCustomIcon(cicon);
                    var geod = geodesc;
    ...
    this.createCookie = function(a, b, persistent) {
        var c, d, e, space;
        c = "";
        e = this.readCookie(a);
        if (e == null) e = "";
        ...
        if (agent[agentParams.browserType] === browserTypes.internetExplorer) {
    ...
    this.eraseCookie = function(a) { document.cookie = a + "=" + null + "; expires=Tues, 31 Dec 2000 23:59:59 GMT; path=/"; };
    ...
    this.getUserValues = function() {
        try {
            var cookie = this.readCookie("sfmve");
            if (cookie == null || cookie == "")
                this.createCookie("sfmve", params.join(","), true);
            else {
                var sp = cookie.split(",");
                if (sp.length > 9) {
                    this.eraseCookie("sfmve");
                    this.createCookie("sfmve", params, true);
                }
                else
                    params = sp;
            }
...
function getAgent() {
    var userAgent, poss, pose;
    userAgent = navigator.userAgent.toLowerCase();
    poss = 0;
    pose = 0;
    if (userAgent.indexOf("chrome") > -1)
        agent = new Array(browserTypes.googleChrome, 0);
    else if (userAgent.indexOf("firefox") > -1)
        agent = new Array(browserTypes.firefox, 0);
    else if (userAgent.indexOf("safari") > -1)
        agent = new Array(browserTypes.safari, 0);
    else if (userAgent.indexOf("msie") > -1) {
        poss = userAgent.indexOf("msie") + 4;
        pose = poss;
        do { pose++; } while (userAgent.substr(pose, 1) !== " ");
        agent = new Array(browserTypes.internetExplorer, parseInt(userAgent.substr(poss, pose - poss)));
        if (agent[agentParams.version] === "7") scrollHeight = 20;
        else if (agent[agentParams.version] === "8") scrollHeight = 3;

    }
    else if (userAgent.indexOf("opera") > -1)
        agent = new Array(browserTypes.opera, 0);
};
getAgent();