<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Map.ascx.cs" Inherits="SentinelFM.MapNew_UserControl_Map" %>
    ...
    <script type="text/javascript" language="javascript">
        ...
        $(document).ready(function () {
           ...
           $("#divMapLoadingText").hide();
            ...
            {
               $("#frmVehicleMapfrmVehicleMap").width($(window).width());
               $("#frmVehicleMapfrmVehicleMap").height($(window).height());
            }
            ...
            if ('<%= isFixSize %>' == '1')
            {
                 var mapWidth = $("#frmVehicleMapfrmVehicleMap").width();
                 if ('<%= MapWidth %>' != '0' )
                    mapWidth = <%= MapWidth %>;
                 if ('<%= MapHeight %>' == '0')
                    SetMapSize($("#frmVehicleMapfrmVehicleMap").height(), mapWidth);
                 else 
                    SetMapSize(<%= MapHeight %>, mapWidth);
            }

            if ('<%= hasLandmarks %>' != '0' || '<%= hasGeozone %>' != '0' ) {
                if ('<%= hasLandmarks %>' != '0' )
                   eval('recursiveObject_l = <%= LankMarkData %>');
            ...
            myMap.map.setZoomIndex(<%=zoomLevel %>);
...
//                });
            ...
            var loadingY = $("#frmVehicleMapfrmVehicleMap").offset().top;
        ...
        $(window).resize(function () {
            if (<%= isFixSize %> == 0 )
            {
                if (<%=isBigMap %> == 0)
                   ...
                   //SetMapSize($("#frmVehicleMapfrmVehicleMap").height(), $(window).width());
                else 
                  SetMapSize($(window).height() - 10, $(window).width() -10);
            }
        });
...
//                RecursiveShowLandmarks();
       ...
       try{
            myMap.assetLayer.clear();
            myMap.historyLayer.setPoints([]);
    ...
    <div id="mapContainer" style="border: 1px solid black; height:100%; width:100%" >
    </div>
        ...
        <ul id="jsddm">
            ...
            <li class="dropdown">
                ...
                <ul>
                    ...
                    <li>
                        <input id="lsdMapping" type="checkbox" onchange="return lsdMapping_click(this)" style="display:none;"  /><span id="spanlsdMapping" style="display:none;">LSD Mapping</span>
                    </li>
