...
{
    public partial class Map_frmSensorsInfo_Reefer : SensorInfoPage
    {
        ...
        public string ResTempratureAllowedInNumbers = "";
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    if (this.LicensePlate == "Multi")
                    {
                        boxid = sn.Cmd.ArrVehicle[sn.Cmd.ArrVehicle.Count - 1];
                        sn.Cmd.ArrVehicle.RemoveAt(sn.Cmd.ArrVehicle.Count - 1);
                        chkSendToFleet.Visible = false;
                        chkSendToVehicles.Visible = true;
                        chkSendToVehicles.Checked = true;
                        chkSendToVehicles.Enabled = false;
                        cmdVehicleInfo.Enabled = false;
                        cmdSettings.Enabled = false;
                        cmdReefer.Enabled = false;
                        cmdServices.Enabled = false;
                        cmdUnitInfo.Enabled = false;
                        chkScheduleTask.Enabled = false;
                        lblKeepTry.Enabled = false;
                        cboSchInterval.Enabled = false;
                    }
                    else
                    {
                        chkSendToVehicles.Visible = false;
                    }
                    ...
                    if (sn.Cmd.SelectedVehicleLicensePlate != "Multi")
                    {
                    ...
                    {
                        if (Convert.ToInt16(sn.User.TemperatureZone) == 1)
                        {
                            //txtTemp.Visible = true;
                           
                            tempratureType.Visible = true;                            
                        ...
                        else
                        {
                            ...
                            if (Convert.ToInt16(sn.User.TemperatureZone) != 0)
                            {
                                for (int i = 1; i <= Convert.ToInt16(sn.User.TemperatureZone); i++)
                                {
                                    ddlTemp.Items.Add(new ListItem(i.ToString(), i.ToString()));
                                }
                            }
        ...
        private void CboCommand_Fill(string LicensePlate)
        {
            try
            {
                ...
                {
                    if (objUtil.ErrCheck(dbv.GetMultiVehicleCommandsXMLByLang(sn.UserID, sn.SecId, boxid, CultureInfo.CurrentUICulture.TwoLetterISOLanguageName, ref xml), false))
                        if (objUtil.ErrCheck(dbv.GetMultiVehicleCommandsXMLByLang(sn.UserID, sn.SecId, boxid, CultureInfo.CurrentUICulture.TwoLetterISOLanguageName, ref xml), true))
                        {
                            System.Diagnostics.Trace.WriteLineIf(AppConfig.tsMain.TraceError, VLF.CLS.Util.TraceFormat(VLF.CLS.Def.Enums.TraceSeverity.Error, "No command for vehicle: " + LicensePlate + " User:" + sn.UserID.ToString() + " Form:" + Page.GetType().Name));
                            cboCommand.Items.Clear();
                            cboCommand.Items.Insert(0, new ListItem((string)base.GetLocalResourceObject("cboCommand_Item_0"), "-1"));
                ...
                {
                    ds.Tables[0].Select();
                    dt = ds.Tables[0].Select("BoxProtocolTypeId=" + sn.Cmd.DsProtocolTypes.Tables[0].Rows[0]["BoxProtocolTypeId"].ToString()).CopyToDataTable();

                    if (sn.Cmd.DsProtocolTypes.Tables.Count != 0 && sn.Cmd.DsProtocolTypes.Tables[0].Rows.Count > 1)
                    {
                ...
                else
                {
                    ...
                    if (optCommMode.SelectedValue == "1")
                    {
                        ds.Tables[0].Select();
                        dt = ds.Tables[0].Select("ChPriority=1 and (BoxCmdOutTypeName LIKE '%Reefer%' or BoxCmdOutTypeName = 'Get Box Status')").CopyToDataTable();
                    }
                    else
                    {
                        ds.Tables[0].Select();
                        dt = ds.Tables[0].Select("ChPriority=0 and (BoxCmdOutTypeName LIKE '%Reefer%' or BoxCmdOutTypeName = 'Get Box Status')").CopyToDataTable();
                    }
            ...
            try
            {
                ...
                switch (Convert.ToInt32(this.cboCommand.SelectedItem.Value))
                {
                    ...
                    case (Int32)VLF.CLS.Def.Enums.CommandType.Reefer_Change_Set_Point:
                        //CboUnAssGeoZone_Fill();
                        this.tblSetTemp.Visible = true;
                        break;
                    case (Int32)VLF.CLS.Def.Enums.CommandType.Reefer_Change_Operating_Mode:
                        //CboUnAssGeoZone_Fill();
                        this.tblChangeOperatingMode.Visible = true;
                        break;
        ...
        protected void cmdSendCommand_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                {
                    boxId = new int[1];
                    boxId[0] = sn.Cmd.BoxId;
                }
                else
                {
                    boxId = new int[boxid.Split(',').Length];
                    int i = 0;
                    foreach (string s in boxid.Split(','))
                    {
                        if (!String.IsNullOrEmpty(s))
                        {
                            boxId[i] = Convert.ToInt32(s);
                            i++;
                        }
                    }
                }
                foreach (int boxIdx in boxId)
                {
                    if (boxIdx == 0)
                        continue;
                    sn.Cmd.BoxId = boxIdx;
                    ...
                    switch (cmdType)
                    {
                        ...
                        case (Int32)VLF.CLS.Def.Enums.CommandType.Reefer_Change_Set_Point:
                           Int32 Temperature = 0;
                            ...
                            if (sn.User.TemperatureZone != "")
                            {
                                ...
                                if (Convert.ToInt32(sn.User.TemperatureZone) == 1)
                                {
                                    ...
                                    //Temperature = Convert.ToInt32(Convert.ToSingle(this.txtTemp.Text));
                                    if (sn.User.TemperatureType == "Fahrenheit")
                                    {
                                        Temperature = Convert.ToInt32(Convert.ToSingle(this.txtbxfahrenheit.Text));
                                    }
                                    else{
                                        Temperature = Convert.ToInt32(Convert.ToSingle(this.txtbxcelsius.Text));
                                    }
                                    paramList += VLF.CLS.Util.MakePair(VLF.CLS.Def.Const.keyTemperature, Temperature.ToString());
                                ...
                                else
                                {
                                    ...
                                    paramList += VLF.CLS.Util.MakePair(VLF.CLS.Def.Const.keyTemperature, Temperature.ToString());
                                    paramList += VLF.CLS.Util.MakePair("ZONE", ddlTemp.SelectedValue);
                                }
                            ...
                            this.tblSetTemp.Visible = false;
                            break;

                        case (Int32)VLF.CLS.Def.Enums.CommandType.Reefer_Initiate_Defrost: //111:   // Reefer Initiate De-frost it was Reefer_Zone_Selection
                            paramList += VLF.CLS.Util.MakePair("ZONE", "1");
                        ...
                        case (Int32)VLF.CLS.Def.Enums.CommandType.Reefer_Change_Operating_Mode:
                            ...
                            Int32 OpMode = 0;
                            OpMode = Convert.ToInt32(Convert.ToSingle(this.cboOperationMode.SelectedValue));

                            paramList += VLF.CLS.Util.MakePair("OP_MODE", OpMode.ToString());

                            this.tblChangeOperatingMode.Visible = false;
                            break;
                    ...
                    {
                        this.lblMessage.Visible = true;
                        if (SyncGeoZoneforVehicle(Convert.ToInt32(lblVehicleId.Text), boxIdx))
                    ...
                    if (objUtil.ErrCheck(dbl.SendCommand(sn.UserID, sn.SecId, DateTime.Now, boxIdx, sn.Cmd.CommandId, paramList, ref ProtocolId, ref CommModeId, ref cmdSent, ref sessionTimeOut), false, System.Globalization.CultureInfo.CurrentUICulture.TwoLetterISOLanguageName, ref errMsg))
                        if (errMsg == "")
                        {
                            if (objUtil.ErrCheck(dbl.SendCommand(sn.UserID, sn.SecId, DateTime.Now, boxIdx, sn.Cmd.CommandId, paramList, ref ProtocolId, ref CommModeId, ref cmdSent, ref sessionTimeOut), true, System.Globalization.CultureInfo.CurrentUICulture.TwoLetterISOLanguageName, ref errMsg))
                            {
                                this.lblMessage.Visible = true;
                                ...
                                if (errMsg != "")
                                    ...
                                    this.lblMessage.Text = (string)base.GetLocalResourceObject("lblMessage_Text_SendCommandFailedUsing") + " BoxId:" + boxIdx.ToString() + ", " + (VLF.CLS.Def.Enums.CommMode)sn.Cmd.CommModeId;
                        ...
                        )
                    {
                        ...
                        if (objUtil.ErrCheck(dbl.GetCommandStatus(sn.UserID, sn.SecId, boxIdx, sn.Cmd.ProtocolTypeId, ref cmdStatus), false))
                            if (objUtil.ErrCheck(dbl.GetCommandStatus(sn.UserID, sn.SecId, boxIdx, sn.Cmd.ProtocolTypeId, ref cmdStatus), true))
                            {
                                this.lblMessage.Visible = true;
                        ...
                        sessionTimeOut = 0;
                        if (objUtil.ErrCheck(dbl.SendCommand(sn.UserID, sn.SecId, DateTime.Now, boxIdx, sn.Cmd.CommandId, sn.Cmd.CommandParams, ref AlternativeProtocol, ref CommModeId, ref cmdSent, ref sessionTimeOut), false))
                            if (objUtil.ErrCheck(dbl.SendCommand(sn.UserID, sn.SecId, DateTime.Now, boxIdx, sn.Cmd.CommandId, sn.Cmd.CommandParams, ref AlternativeProtocol, ref CommModeId, ref cmdSent, ref sessionTimeOut), true))
                            {
                                this.lblMessage.Visible = true;
                                this.lblMessage.Text = (string)base.GetLocalResourceObject("lblMessage_Text_SendCommandFailedUsing") + " " + (VLF.CLS.Def.Enums.CommMode)sn.Cmd.CommModeId;
                                System.Diagnostics.Trace.WriteLineIf(AppConfig.tsMain.TraceError, VLF.CLS.Util.TraceFormat(VLF.CLS.Def.Enums.TraceSeverity.Error, "Sent command failed.  User:" + sn.UserID.ToString() + " Form:" + Page.GetType().Name));
                ...
                {
                    this.lblMessage.Visible = true;
                    this.lblMessage.Text = "The command has been sent.";
                    return;
                }
                else
                {
        ...
        private void GetVehicleInfo(string LicensePlate)
        {
            try
            {
                ...
                this.lblLastComm.Text = Convert.ToDateTime(rowItem["LastCommunicatedDateTime"]).ToString();
        ...
        private void HideAllTables()
        {
            ...
            tblSetTemp.Visible = false;
            tblChangeOperatingMode.Visible = false;
