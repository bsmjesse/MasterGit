...
{
    public partial class AlarmDetails : System.Web.UI.Page
    {
                        ...
                        case "loadData":
                            {
                                ...
                                {
                                    GuiSecurity(this);
                                    string resultXML = AlarmInfoLoad_NewTZ(AlarmId);
                        ...
                        case "acceptAlarm":
                            {
                                ...
                                {
                                    int AlarmId = Convert.ToInt32(Session["AlarmId"]);
                                    if (acceptAlarm_NewTZ(AlarmId, Request["ExtraNotes"].ToString().Replace("'", "''")))
                                    {
                                        result = "{\"success\":true}";//serializer.Serialize(true);
                                    }
                                    else
                                    {
                                        result = "{\"success\":false, \"msg\":\"Error in accepting the alarm.. Please contact BSM Support...\"}"; //serializer.Serialize("Error in accepting the alarm.. Please contact BSM Support...");
                                    }
                                }
                                catch (Exception Ex)
                                {
        ...
        private string AlarmInfoLoad_NewTZ(int AlarmId)        
        {
            ...
            try
            {
                ...
                //this.BoxId.Text = ds.Tables[0].Rows[0]["BoxId"].ToString().TrimEnd();
                ...
                //this.Speed.Text = ds.Tables[0].Rows[0]["Speed"].ToString().TrimEnd();
                //this.Heading.Text = ds.Tables[0].Rows[0]["Heading"].ToString().TrimEnd();
        ...
        private string AlarmInfoLoad(int AlarmId)
            //void AlarmInfoLoad(int AlarmId)
        {
        ...
        protected bool acceptAlarm_NewTZ(int alarmID, string notes)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(notes))
                {
                    ...
                    if (objUtil.ErrCheck(alarms.AcceptCurrentAlarm(sn.UserID, sn.SecId, alarmID), false))
                        if (objUtil.ErrCheck(alarms.AcceptCurrentAlarm(sn.UserID, sn.SecId, alarmID), true))
                        {
                    ...
                    //if (objUtil.ErrCheck(alarms.CloseCurrentAlarmWithNotes(sn.UserID, sn.SecId, Convert.ToInt32(ViewState["AlarmId"]), this.ExtraNotes.Text), false))
                    // if (objUtil.ErrCheck(alarms.CloseCurrentAlarmWithNotes(sn.UserID, sn.SecId, Convert.ToInt32(ViewState["AlarmId"]), this.ExtraNotes.Text), true))
                    if (objUtil.ErrCheck(alarms.CloseCurrentAlarmWithNotes(sn.UserID, sn.SecId, alarmID, notes), false))
                        if (objUtil.ErrCheck(alarms.CloseCurrentAlarmWithNotes(sn.UserID, sn.SecId, alarmID, notes), true))
                        {
        ...
        protected bool acceptAlarm(int alarmID,string notes)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(notes))
                {
                    ...
                    sn.Map.LoadAlarmsXML(sn, CultureInfo.CurrentUICulture.TwoLetterISOLanguageName, ref str, ref AlarmCount);
                    sn.Map.AlarmsXML = str;
                    sn.Map.AlarmCount = AlarmCount;
        ...
        //        vehicleDic.Add("BoxId", this.BoxId.Text);
        //        vehicleDic.Add("Description", this.VehicleDescription.Text);
        //        vehicleDic.Add("StreetAddress", this.StreetAddress.Text);
        //        vehicleDic.Add("OriginDateTime", this.TimeCreated.Text);
        //        //vehicleDic.Add("convertedDate", this.TimeCreated.Text); //TODO: Get actual converted date.
        //        //vehicleDic.Add("convertedDisplayDate", this.TimeCreated.Text); //TODO: Get actual converted display date.
        //        vehicleDic.Add("Speed", this.Speed.Text);
        //        vehicleDic.Add("VehicleId", this.VehicleId.Text);
        //        vehicleDic.Add("Latitude", this.Latitude.Text);
        //        vehicleDic.Add("Longitude", this.Longitude.Text);
        //        vehicleDic.Add("icon", "RedCircle.ico");
        //        //vehicleDic.Add("CustomSpeed", this.TimeCreated.Text); //TODO: Get actual converted speed.
        //        vehicleDic.Add("MyHeading", this.Heading.Text);
        //        vehicleDic.Add("VehicleStatus", this.AlarmDescription.Text);
        ...
        //        //Response.Write("<script language='javascript'>window.close()</script>");
        //        string response = "<SCRIPT Language='javascript'>try { var winparent = window.opener; if ( winparent == null )";
        //        response = response + "{ winparent = window.parent; } if( winparent!=null ){ winparent.MapAlarm(";
        //        response = response + parameters + "); } } catch(err) {console.log('Error: ' + err.message);}";
        //        response = response + "window.close(); </script>";
