...
namespace SentinelFM.Map
{
    ...
    /// </summary>
    public partial class mapSearch : SentinelFMBasePage
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                LocalizationLayer.GUILocalizationLayer.GUILocalizeForm(ref mapSearchForm, System.Globalization.CultureInfo.CurrentUICulture.TwoLetterISOLanguageName);
        ...
        protected void cmdGoTo_Click(object sender, System.EventArgs e)
        {
                 ...
                 if ((this.cboSearchType.SelectedItem.Value == "4"))
            {
                ...
                for (int i = 0; i < dgMultiplyVehicle.Items.Count; i++)
                {
                    ...
                    if (ch.Checked)
                    {
                        sb.Append(dgMultiplyVehicle.Items[i].Cells[0].Text);
                        sb.Append(",");
                        checkedVehicles++;
                    }
                }

                if (checkedVehicles == 0)
                {
                    this.lblMessage.Visible = true;
                    this.lblMessage.Text = "Please select at least one record.";
                    return;
                }
                string ret = sb.ToString();
                ret = ret.Substring(0,ret.LastIndexOf(","));
                sb = new StringBuilder();
                sb.Append("<SCRIPT Language='javascript'>try { var winparent = window.opener; if ( winparent == null )");
                sb.Append("{ winparent = window.parent; } if( winparent!=null ){ winparent.mapSelectedVehIDs('");                
                sb.Append(ret);                
                sb.Append("'); } } catch(err) {console.log('Error: ' + err.message);}");
            ...
            {              
                ...
                if ((ViewState["VehicleId"] != null) && (ViewState["VehicleId"].ToString() != ""))
                {
                    ...
                    sb.Append("<SCRIPT Language='javascript'>try { var winparent = window.opener; if ( winparent == null )");
                    sb.Append("{ winparent = window.parent; } if( winparent!=null ){ winparent.mapSelectedVehIDs(");
                    sb.Append(ViewState["VehicleId"].ToString());
                    sb.Append("); } } catch(err) {console.log('Error: ' + err.message);}");
                    sb.Append("window.close(); </script>");
                    string response = sb.ToString();
                    Response.Write(response);
                    //Response.Write(window.opener.location.href='../NewMap.aspx'; </SCRIPT>");
            ...
            if ((this.cboSearchType.SelectedItem.Value == "2") || (this.cboSearchType.SelectedItem.Value == "5"))
            {
