...
namespace SentinelFM
{
    ...
    /// </summary>
    public partial class frmMapSearch : SentinelFMBasePage
    {
       ...
       //Map24 Geocoder
        ...
        private void DgByAddress_Fill()
        {
            try
            {
                ...
                {
                   if (objUtil.ErrCheck(dbf.GetVehiclesLastKnownPositionInfoNearestToLatLon(sn.UserID, sn.SecId, sn.User.OrganizationId, Convert.ToInt32(this.cboIntrFleet.SelectedItem.Value), Convert.ToInt32(this.cboIntrDistance.SelectedItem.Value), Convert.ToDouble(this.txtIntrY.Text), Convert.ToDouble(this.txtIntrX.Text), ref xml), false))
                      if (objUtil.ErrCheck(dbf.GetVehiclesLastKnownPositionInfoNearestToLatLon(sn.UserID, sn.SecId, sn.User.OrganizationId, Convert.ToInt32(this.cboIntrFleet.SelectedItem.Value), Convert.ToInt32(this.cboIntrDistance.SelectedItem.Value), Convert.ToDouble(this.txtIntrY.Text), Convert.ToDouble(this.txtIntrX.Text), ref xml), true))
                      {
                         dgMultiplyVehicle.DataSource = null;
                         dgMultiplyVehicle.DataBind();
                         return;
                ...
                try
                {
                   geoPoint1.Latitude = Convert.ToDouble(this.txtIntrY.Text);
                   geoPoint1.Longitude = Convert.ToDouble(this.txtIntrX.Text);
                }
                catch
                {
                }
        ...
        protected void cmdFindAddress_Click(object sender, EventArgs e)
        {
          ...
          //  string strAddress1 = "";
            ...
            //}
             ...
             xml = "<Geocode_Addresses>";
              xml += "<Geocode_Table>";
              xml += "<Address>" + resolvedAddress + "</Address>";
                 xml += "<Latitude>" + Y.ToString()  + "</Latitude>";
                 xml += "<Longitude>" + X.ToString() + "</Longitude>";
                 xml += "</Geocode_Table>";
       ...
       private void SaveShowCheckBoxes()
       {
        ...
        protected void dgAddress_ItemCommand(object source, DataGridCommandEventArgs e)
        {
               ...
               ClientMapProxy map = new VLF.MAP.ClientMapProxy(sn.User.MapEngine);
               map.MapCenterLongitude = Convert.ToDouble(e.Item.Cells[2].Text);
               map.MapCenterLatitude = Convert.ToDouble(e.Item.Cells[1].Text);
               sn.History.MapCenterLongitude = map.MapCenterLongitude.ToString() ;
               sn.History.MapCenterLatitude = map.MapCenterLatitude.ToString();
               MapZoomLevel mapZoomLevel=MapZoomLevel.StreetLevelHigh2;
               map.Zoom(mapZoomLevel);
               sn.History.ImageDistance = map.ImageDistance.ToString();
               sn.History.MapScale = map.MapScale.ToString();
               sn.History.MapSearch = true;
               ...
               string strUrl = "<script language='javascript'>window.opener.document.location.href=" + ViewState["frm"].ToString() + ";</script>";
