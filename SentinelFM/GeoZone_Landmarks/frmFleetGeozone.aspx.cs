...
{
    public partial class frmFleetGeozone : SentinelFMBasePage
    {
        ...
        VLF.PATCH.Logic.PatchOrganizationGeozone pog;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            pog = new VLF.PATCH.Logic.PatchOrganizationGeozone(sConnectionString);
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (fleetId != -1)
                {
                    ...
                    lstUnAssGeozones_Fill(fleetId);
                    lstAssGeozones_Fill(fleetId);
                }
        ...
        protected void cboFleet_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Convert.ToInt32(cboFleet.SelectedItem.Value) != -1)
            {
                ...
                lstUnAssGeozones_Fill(Convert.ToInt32(cboFleet.SelectedItem.Value));
                lstAssGeozones_Fill(Convert.ToInt32(cboFleet.SelectedItem.Value));
            }
            else
            {
        ...
        protected void hidOrganizationHierarchyPostBack_Click(object sender, System.EventArgs e)
        {
            ...
            {
                this.tblVehicles.Visible = true;
                lstUnAssGeozones_Fill(Convert.ToInt32(hidOrganizationHierarchyFleetId.Value));
                lstAssGeozones_Fill(Convert.ToInt32(hidOrganizationHierarchyFleetId.Value));
            }
            else
            {
                this.lstAss.Items.Clear();
        ...
        protected void cmdAdd_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                if (this.lstUnAss.SelectedIndex == -1)
                {
                    ...
                    this.lblMessage.Text = "Please select a Geozone";  //"Please select a vehicle.";
                    return;
                }
                ...
                foreach (ListItem li in lstUnAss.Items)
                {
                    if (li.Selected)
                    {
                        pog.AssignGeozoneToFleet(sn.User.OrganizationId, Convert.ToInt32(li.Value), fleetId);
                    }
            ...
            {
                System.Diagnostics.Trace.WriteLineIf(AppConfig.tsMain.TraceError, VLF.CLS.Util.TraceFormat(VLF.CLS.Def.Enums.TraceSeverity.Error, Ex.Message.ToString() + " User:" + sn.UserID.ToString() + " From:frmFleetGeozone.aspx"));
            }
        }
        ...
        protected void cmdClearAll_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (ListItem li in lstAss.Items)
                {
                    ...
                    pog.UnassignGeozoneToFleet(sn.User.OrganizationId, Convert.ToInt32(li.Value), fleetId);                   
        ...
        private void lstUnAssGeozones_Fill(int fleetId)
        {
            ...
            try
            {
                DataSet dsAllUnassignedToFleetGeozones = new DataSet();
                dsAllUnassignedToFleetGeozones = pog.GetAllUnassignedToFleetGeozonesInfo(sn.User.OrganizationId, fleetId, sn.UserID);

                if (sn.User.ControlEnable(sn, 93))
                {
                    dt = dsAllUnassignedToFleetGeozones.Tables[0].Clone();

                    string filter = String.Format("Public=false AND CreateUserID={0}", sn.UserID.ToString());
                    drCollections = dsAllUnassignedToFleetGeozones.Tables[0].Select(filter);
                ...
                {
                    this.lstUnAss.DataSource = dsAllUnassignedToFleetGeozones;
                    this.lstUnAss.DataBind();
                }     
            }
            catch (NullReferenceException Ex)
        ...
        private void lstAssGeozones_Fill(int fleetId)
        {
            DataTable dt = new DataTable();
            DataRow[] drCollections = null;
            ...
            try
            {
                DataSet dsAllAssignedToFleetGeozones = new DataSet();
                dsAllAssignedToFleetGeozones = pog.GetAllAssignedToFleetGeozonesInfo(sn.User.OrganizationId, fleetId);
                

                if (sn.User.ControlEnable(sn, 93))
                {
                    dt = dsAllAssignedToFleetGeozones.Tables[0].Clone();

                    string filter = String.Format("Public=false AND CreateUserID={0}", sn.UserID.ToString());
                    drCollections = dsAllAssignedToFleetGeozones.Tables[0].Select(filter);
                ...
                {
                    this.lstAss.DataSource = dsAllAssignedToFleetGeozones;
                    this.lstAss.DataBind();
                }     
            }
            catch (NullReferenceException Ex)
