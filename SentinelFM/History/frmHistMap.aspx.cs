...
{
   public partial class History_frmHistMap : SentinelFMBasePage
   {
      ...
      protected void Page_Load(object sender, EventArgs e)
      {
          try
          {
              ...
              if (Request.QueryString["clientWidth"] != null)
                      sn.User.ScreenWidth = Convert.ToInt32(Request.QueryString["clientWidth"])+265 ;

              imageW = Convert.ToInt32(sn.User.ScreenWidth) - 345;
               ...
               if (sn.History.ShowStops || sn.History.ShowStopsAndIdle || sn.History.ShowIdle)
               {
                  if (map.Vehicles.Count > 0)
                  {
                    ...
                    // sn.Map.VehiclesMappings += "<area onmouseover=\"javascript:toolTip('" + 1 + "');document.myform.imgMap.style.cursor='crosshair'\"  onmouseout=\"javascript:hide();document.myform.imgMap.style.cursor='default'\" shape=\"RECT\" coords=\"" + StartX.ToString() + "," + StartY.ToString() + "," + EndX.ToString() + "," + EndY.ToString() + "\">";
                     ...
                     areaMaps += "locationMap[\"" + 1 + "\"] = [" + string.Format("{0},{1},{2},{3}", StartX, StartY, EndX, EndY) + "];";
                     areaMaps += "</script>";
                  ...
                  for (int i = 0; i < iconCollection.Count; i++)
                  {
                     if ((iconCollection[i].X != -1) && (iconCollection[i].Y != -1))
                     {
                        ...
                        if (sn.History.ShowTrips)
                            drArr = sn.History.DsHistoryInfo.Tables[1].Select("OriginDateTime='" + iconCollection[i].IconLabel + "'");
                         else
                            drArr = sn.History.DsHistoryInfo.Tables[0].Select("OriginDateTime='" + iconCollection[i].IconLabel + "'");
                        ...
                        {
                           if (sn.Map.VehiclesToolTip.Length == 0)
                              sn.Map.VehiclesToolTip += "<B>" + " [" + rowItem["MyDateTime"].ToString().TrimEnd() + "]:</B><FONT style='COLOR: Purple'>" + " "+ rowItem["MsgDetails"].ToString().TrimEnd() +  "</FONT><BR> <FONT style='TEXT-DECORATION: underline' > Address:</FONT> " + rowItem["StreetAddress"].ToString().TrimEnd() + "<BR> <FONT style='TEXT-DECORATION: underline' > Speed:</FONT> " + rowItem["Speed"].ToString().TrimEnd() + " " + UnitOfMes + "<BR>\",";
                           else
                               sn.Map.VehiclesToolTip += "\"<B>" + " [" + rowItem["MyDateTime"].ToString().TrimEnd() + "]:</B><FONT style='COLOR: Purple'>" + " " + rowItem["MsgDetails"].ToString().TrimEnd() + "</FONT><BR> <FONT style='TEXT-DECORATION: underline' > Address:</FONT> " + rowItem["StreetAddress"].ToString().TrimEnd() + "<BR> <FONT style='TEXT-DECORATION: underline' > Speed:</FONT> " + rowItem["Speed"].ToString().TrimEnd() + " " + UnitOfMes + "<BR>\",";
                        }
                        else
      ...
      //        {
      //            VLF.MAP.GeoPoint geoPoint=new GeoPoint();  
      //            geoPoint.Latitude =map.Vehicles[i].Latitude;
      //            geoPoint.Longitude  =map.Vehicles[i].Longitude;
      //            HistoryItem vehicleItem = new HistoryItem();
      //            vehicleItem.Timestamp = Convert.ToDateTime(map.Vehicles[i].IconLabel);
      //            vehicleItem.Latitude = map.ConvertLatLonToPx(geoPoint).Y;
      //            vehicleItem.Longitude =map.ConvertLatLonToPx(geoPoint).X ;
      ...
      //    s.AppendLine(CreateJSmapInfoArray(sn.History.ImgPath, map.NorthEastCorner.Latitude, map.NorthEastCorner.Longitude, map.SouthWestCorner.Latitude  , map.SouthWestCorner.Longitude   , map.MapHeight , map.MapWidth,map.MapScale));
      ...
      string CreateJSmapInfoArray(string mapImageFilePath, double upperLatitude, double upperLongitude, double lowerLatitude, double lowerLongitude, int imageHeight, int imageWidth, double scale)
      {
          StringBuilder s = new StringBuilder(mapArray);
          s.AppendFormat(" \"{0}\", {1}, {2}, {3}, {4}, {5}, {6}, {7});", mapImageFilePath, upperLatitude, upperLongitude, lowerLatitude, lowerLongitude, imageHeight, imageWidth, scale);
          return s.ToString();
      }
      ...
      string CreateJShistoryItemArray(HistoryItem item)
      {
          ...
          s.AppendFormat(" \"{0}\", {1}, {2}, {3}, false)", item.Timestamp.ToString("MM/dd/yyyy HH:mm:ss"), item.Latitude, item.Longitude, item.ImageIndex);
          return s.ToString();
      }
