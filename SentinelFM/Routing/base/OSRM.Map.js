...
init: function() {
	...
	OSRM.G.SpeedDispute = new SpeedDispute(OSRM.G.map);
	...
	OSRM.G.map.on('dblclick', OSRM.Map.click );
...
contextmenu: function (e) {
    ...
    content += '<div style="color: #999999">' + e.latlng + '</div></div>';
    ...
    g.ajax({
        type: 'POST',
        url: '../MapNew/NewMapGeozoneLandmark.asmx/GetGeoInfo',
        data: JSON.stringify({ lon: e.latlng.lng, lat: e.latlng.lat }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: true,
        success: function (msg) {
            var r = eval('(' + msg.d + ')');
            g('#contextMenuStreetAddress').html(r.StreetAddress);
            g('#contextPostedSpeed').html(r.RoadSpeed);
            g('#contextFunctionClass').html(r.FunctionClass);
            g('#contextSpeedCategory').html(r.SpeedCategory);
        },
        error: function (msg) {
...
click: function(e) {
    ...
    ***/
    ...
    OSRM.G.SpeedDispute.MapClick(e);
},
