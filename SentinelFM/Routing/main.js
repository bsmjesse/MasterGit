...
OSRM.parseParameters = function(){
	...
	if( params.destination ) {
		...
		if (params.destination_name) {
		    if (document.getElementById("gui-input-target").value.length < 1) {
	...
	if( params.positions ) {
		...
		if (OSRM.G.RouteProvider == "NavTEQ") {
		    OSRM.GUI.setRoutingEngine(params.active_routing_engine || OSRM.DEFAULTS.NAVTEQ_ROUTING_ENGINES);
		} else {
