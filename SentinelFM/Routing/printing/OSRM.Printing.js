...
showOSRMDescription: function (response) {
    var route_name = "(";
    ...
    if (OSRM.Browser.IE6_8) {	// tables used for compatibility with legacy IE (quirks mode)
		...
		'<table class="full">' +
		'<tr class="row">' +
		'<td class="center description-header-label nowrap">' + OSRM.loc("GUI_START") + ': </td>' +
		'<td class="left description-header-content stretch">' + document.getElementById("gui-input-source").value + '</td>' +
		'</tr>' +
		'<tr class="row">' +
		'<td class="center description-header-label nowrap">' + '↓' + '</td>' +
		'<td class="left description-header-label stretch">' + route_name + '</td>' +
		'</tr>' +
		'<tr class="row">' +
		'<td class="center description-header-label nowrap">' + OSRM.loc("GUI_END") + ': </td>' +
		...
		'<table class="full">' +
		'<tr class="row">' +
		'<td class="left description-header-label nowrap">' + OSRM.loc("DISTANCE") + ': </td>' +
		'<td class="left description-header-content stretch">' + OSRM.Utils.toHumanDistance(response.route_summary.total_distance) + '</td>' +
		'</tr>' +
		'<tr class="row">' +
		'<td class="left description-header-label nowrap">' + OSRM.loc("DURATION") + ': </td>' +
		'<td class="left description-header-content stretch">' + OSRM.Utils.toHumanTime(response.route_summary.total_time) + '</td>' +
		'</tr>' +
		'</table>' +
		'</td>' +
		...
		'<div class="full">' +
		'<div class="row">' +
		'<div class="center description-header-label nowrap">' + OSRM.loc("GUI_START") + ': </div>' +
		'<div class="left description-header-content stretch">' + document.getElementById("gui-input-source").value + '</div>' +
		'</div>' +
		'<div class="row">' +
		'<div class="center description-header-label nowrap">' + '↓' + '</div>' +
		'<div class="left description-header-label stretch">' + route_name + '</div>' +
		'</div>' +
		'<div class="row">' +
		'<div class="center description-header-label nowrap">' + OSRM.loc("GUI_END") + ': </div>' +
		...
		'<div class="full">' +
		'<div class="row">' +
		'<div class="left description-header-label nowrap">' + OSRM.loc("DISTANCE") + ': </div>' +
		'<div class="left description-header-content stretch">' + OSRM.Utils.toHumanDistance(response.route_summary.total_distance) + '</div>' +
		'</div>' +
		'<div class="row">' +
		'<div class="left description-header-label nowrap">' + OSRM.loc("DURATION") + ': </div>' +
		'<div class="left description-header-content stretch">' + OSRM.Utils.toHumanTime(response.route_summary.total_time) + '</div>' +
		'</div>' +
		'</div>' +
		'</div>' +
...
show: function (response) {    
    var bodyInfo = OSRM.Printing.showOSRMDescription(response);
    var body = bodyInfo.body;
    var header = bodyInfo.header;
	...
	// draw map
    var tile_server_id = 1;//OSRM.G.map.getActiveLayerId();
...
showNavTeqDescription: function (response) {
    var addressCollections = addressesCollection.split('|');
    var  sizej = addressCollections.length;
    var route_name = "";    
    for (var j = 1; j < sizej - 1; j++) {
        var addressInfo = addressCollections[j].split('@');
        route_name += "(<span style='white-space:nowrap;'>" + addressInfo[0] + "</span>)<br />";
    }
    var destInfo = addressCollections[sizej - 1].split('@');
    var dest = destInfo[0];
    // create header
    var header;
    if (OSRM.Browser.IE6_8) {	// tables used for compatibility with legacy IE (quirks mode)
		...
		'<tr class="row">' +
		'<td class="center description-header-label nowrap">' + OSRM.loc("GUI_END") + ': </td>' +
		'<td class="left description-header-content stretch">' + dest + '</td>' +
		'</tr>' +
		'</table>' +
		'</td>' +
		...
		'<tr class="row">' +
		'<td class="left description-header-label nowrap">' + OSRM.loc("DISTANCE") + ': </td>' +
		'<td class="left description-header-content stretch">' + OSRM.Utils.toHumanDistance(OSRM.G.NavTEQSummary.Distance) + '</td>' +
		'</tr>' +
		'<tr class="row">' +
		'<td class="left description-header-label nowrap">' + OSRM.loc("DURATION") + ': </td>' +
		'<td class="left description-header-content stretch">' + OSRM.Utils.toHumanTime(OSRM.G.NavTEQSummary.TrafficTime) + '</td>' +
		'</tr>' +
		'</table>' +
		'</td>' +
		...
		'<div class="row">' +
		'<div class="center description-header-label nowrap">' + OSRM.loc("GUI_END") + ': </div>' +
		'<div class="left description-header-content stretch">' + dest + '</div>' +
		'</div>' +
		'</div>' +
		'</div>' +
		...
		'<div class="row">' +
		'<div class="left description-header-label nowrap">' + OSRM.loc("DISTANCE") + ': </div>' +
		'<div class="left description-header-content stretch">' + OSRM.Utils.toHumanDistance(OSRM.G.NavTEQSummary.Distance) + '</div>' +
		'</div>' +
		'<div class="row">' +
		'<div class="left description-header-label nowrap">' + OSRM.loc("DURATION") + ': </div>' +
		'<div class="left description-header-content stretch">' + OSRM.Utils.toHumanTime(OSRM.G.NavTEQSummary.TrafficTime) + '</div>' +
		'</div>' +
		'</div>' +
		'</div>' +
    ...
    /****/
    var i = 0;
    g.each(OSRM.G.NavTEQManeuver, function (key, myVal) {
        //odd or even ?
        var rowstyle = 'description-body-odd';
        if (i % 2 == 0) { rowstyle = 'description-body-even'; }
        ...
        body += '<td class="description-body-directions">';
        body += '<img class="description-body-direction" src="' + OSRM.Printing.BASE_DIRECTORY + OSRM.RoutingDescription._getDrivingInstructionIcon(OSRM.DEFAULTS.NavTeqDirectionAvatars[myVal.Action]) + '" alt="" />';
        body += "</td>";

        // build route description
        body += '<td class="description-body-items">';
        var desc = myVal.Instruction;
        var descs = desc.split(" Go for");
        body += descs[0];
        body += "</td>";

        body += '<td class="description-body-distance">';
        body += (myVal.Length < 1) ? '&nbsp;' : '<b>' + OSRM.Utils.toHumanDistance(myVal.Length) + '</b>';	// fill last entry with a space
        body += "</td>";
...
showNavTEQ: function (response) {   
    var bodyInfo = OSRM.Printing.showNavTeqDescription(response);
    var body = bodyInfo.body;
    var header = bodyInfo.header;
    // put everything in DOM
    var print_window = OSRM.G.printwindow;
...
drawRoute: function(response) {
	...
	if (OSRM.G.RouteProvider == 'OSRM') {
	    ...
	    if (OSRM.G.active_alternative >= response.alternative_geometries.length)	// no redraw if the selected alternative cannot be found
	        return;
	    positions = OSRM.RoutingGeometry._decode(response.alternative_geometries[OSRM.G.active_alternative], OSRM.C.PRECISION);
	}
	else {
	    positions = OSRM.G.NavTeqGeoMetry;
	}
...
printWindowLoaded: function(){
	...
	if (OSRM.G.RouteProvider == 'OSRM') {
	    ...
	    OSRM.Printing.showNavTEQ(OSRM.G.NavTEQRawResponse);
	}
...
getDescriptions: function () {
    if (OSRM.G.RouteProvider == 'OSRM') {
        return OSRM.Printing.showOSRMDescription(OSRM.G.response);
    }
    else {
        return OSRM.Printing.showNavTeqDescription(OSRM.G.NavTEQRawResponse);
    }
}
