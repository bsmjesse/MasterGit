...
OSRM.RoutingDescription = {
    ...
    show: function (response) {
        ...
        //var gpx_button_link = '<input type="button" id="gpx_button" value="Save">';
        //var edit_route_link = '<input type="button" id="btnRoutersList" name="btnRouterList" value="Edit"/>';
        var gpx_link = '<a class="display-link" onClick="document.location.href=\'' + OSRM.G.active_routing_server_url + query_string + '&output=gpx\';">[' + OSRM.loc("GPX_FILE") + ']</a>';
        var gpx_button_link = '<a class="display-link" id="gpx_button">[Save]</a>&nbsp;';
        var edit_route_link = '<a class="display-link" id="btnRoutersList">[Edit]</a>&nbsp;';
        ...
        OSRM.RoutingDescription._dynamicAddControls()
        ...
        body += '</table>';
        body += '<br>';
        //body += '<div class="right header-content">' + gpx_button_link + '</div>';
        //body += '<div class="right header-content">' + edit_route_link + '</div>';
        //body += '<div class="right header-content">' + gpx_link + '</div>';
        // build header
        ...
        document.getElementById('txtEditLink').value = OSRM.DEFAULTS.WEBSITE_URL + query_string + '&routeId=' + OSRM.G.NavTeqRouteId;
        document.getElementById('txtQueueString').value = query_string;
    ...
    // handling of routing description for navteq
    showNavTeq: function (instructions, summary) {
        // activate GUI features that need a route
        OSRM.GUI.activateRouteFeatures();

        // compute query string
        var query_string = '?hl=' + OSRM.Localization.current_language;
        for (var i = 0; i < OSRM.G.SortedPoints.length; i++)
            query_string += '&loc=' + OSRM.G.SortedPoints[i].getLat().toFixed(6) + ',' + OSRM.G.SortedPoints[i].getLng().toFixed(6);
        ...
        body += '<table class="description medium-font">';
        var i = 0;
        g.each(instructions, function (mykey2, myval2) {
            var rowstyle = 'description-body-odd';
            if (i % 2 == 0) { rowstyle = 'description-body-even'; }
            ...
            //var pos = positions[response.route_instructions[i][3]];
            body += '<div id="description-' + i + '" class="description-body-item ' + (selected_description == i ? "description-body-item-selected" : "") + '" ' +
            'onclick="OSRM.RoutingDescription.onClickRouteDescription(' + myval2.Position.Latitude + "," + myval2.Position.Longitude + "," + i + ')" ' +
            'onmouseover="OSRM.RoutingDescription.onMouseOverRouteDescription(' + myval2.Position.Latitude + "," + myval2.Position.Longitude + ')" ' +
            'onmouseout="OSRM.RoutingDescription.onMouseOutRouteDescription(' + myval2.Position.Latitude + "," + myval2.Position.Longitude + ')">';
        ...
        //body += '<div class="right header-content">' + gpx_link + '</div>';
        // build header
        header = OSRM.RoutingDescription._buildHeader(OSRM.Utils.toHumanDistance(summary.Distance), OSRM.Utils.toHumanTime(summary.TrafficTime), route_link, gpx_link);
    ...
    // handling of detail for routing description from NavTeq
    showNavTEQDetail: function () {
        // activate GUI features that need a route
        OSRM.GUI.activateRouteFeatures();
        ...
        g.each(OSRM.G.NavTEQManeuver, function(key, myVal){
            ...
            body += '<td class="description-body-directions">';            
            body += '<img class="description-body-direction" src="' + OSRM.RoutingDescription._getDrivingInstructionIcon(OSRM.DEFAULTS.NavTeqDirectionAvatars[myVal.action]) + '" alt=""/>';
            body += '</td>';

            body += '<td class="description-body-items">';            
            body += '<div id="description-' + i + '" class="description-body-item ' + (selected_description == i ? "description-body-item-selected" : "") + '" ' +
            'onclick="OSRM.RoutingDescription.onClickRouteDescription(' + myVal.position.latitude + "," + myVal.position.longitude + "," + i + ')" ' +
            'onmouseover="OSRM.RoutingDescription.onMouseOverRouteDescription(' + myVal.position.latitude + "," + myVal.position.longitude + ')" ' +
            'onmouseout="OSRM.RoutingDescription.onMouseOutRouteDescription(' + myVal.position.latitude + "," + myVal.position.longitude + ')">';
            ...
            ***/
            ...
            body += '<td class="description-body-distance">';
            if (myVal.length > 0) {
                body += '<b>' + OSRM.Utils.toHumanDistance(myVal.length) + '</b>';
            }            
            body += "</td>";
       ...
       // }
        ...
        //body += '<div class="right header-content">' + gpx_link + '</div>';
        // build header
        header = OSRM.RoutingDescription._buildHeader(OSRM.Utils.toHumanDistance(OSRM.G.NavTEQSummary.distance), OSRM.Utils.toHumanTime(OSRM.G.NavTEQSummary.trafficTime), route_link, gpx_link);
