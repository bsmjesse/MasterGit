	...
	public class SecurityManager : System.Web.Services.WebService
	{
      ...
      [WebMethod]
        ...
        public int LoginMD5Extended(string key, string userName, string password, 
                                    string userIp, ref int userId, ref string SID, ref int SuperOrganizationId)
        {
     ...
     public int ReloginMD5(string key, string userName, string password, string userIp, ref string SID)
     {
        ...
        {
            Log(">> ValidateandReloginMD5ByDBName (userName={0}, userIp={1})", userName, userIp);

            try
            {
                 ValidationResult res = SecurityKeyManager.GetInstance().ValidatePasskey(userId, key);
          
                if ((VLF.ERRSecurity.InterfaceError)res == VLF.ERRSecurity.InterfaceError.PassKeyExpired)
                    ReloginMD5ByDBName(userId, key, userName, password, userIp, ref SID);

                return (int)InterfaceError.NoError;
             }
            catch (Exception Ex)
            {
                LogException("<< ValidateandReloginMD5ByDBName : userName={0} EXC={1}", userName, Ex.Message);
                return (int)ASIErrorCheck.CheckError(Ex);
            }
        }
