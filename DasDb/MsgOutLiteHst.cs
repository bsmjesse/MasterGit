...
namespace VLF.DAS.DB
{
   ...
   /// </summary>
   public class MsgOutLiteHst : TblGenInterfaces
   {
      ...
      public MsgOutLiteHst(SQLExecuter sqlExec)
         : base("vlfMsgOutHst", sqlExec)
      {
      }      
      #endregion
      ...
      /// <exception cref="DASDbConnectionClosed">Thrown if connection to database has been closed.</exception>
      /// <exception cref="DASException">Thrown in all other exception cases.</exception>
      public void AppendMsg(CMFOut cMFOut, DateTime dateTime, SByte priority,
                        short dclId, short aslId, int userId)
      {
         ...
         {
            string prefixMsg = "MsgOutLiteHst.AppendMsg ->(SqlException) Unable to add new msg Out with date '" + dateTime + "'.";
            Util.ProcessDbException(prefixMsg, objException);
         }
         catch (DASDbConnectionClosed exCnn)
         {
         ...
         {
            string prefixMsg = "MsgOutLiteHst.AppendMsg ->(Exception) Unable to add new msg Out with date '" + dateTime + "'.";
            throw new DASException(prefixMsg + " " + objException.Message);
         }
         if (rowsAffected == 0)
         {
            string prefixMsg = "MsgOutLiteHst.AppendMsg ->(rows=0) Unable to add new msg Out with date '" + dateTime + "'.";
