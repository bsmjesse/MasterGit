...
namespace SentinelFM.Admin
{
    ...
    public partial class frmBoxSentCommands : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            string cmdType = txtCommand.Text;

            if (objUtil.ErrCheck(dbh.GetCmdSend(sn.UserID, sn.SecId, fleetId, cmdType, ref xml), false))
                if (objUtil.ErrCheck(dbh.GetCmdSend(sn.UserID, sn.SecId, fleetId, cmdType, ref xml), true))
                {
                    return;
                }
            ...
            {
                ds.Tables[0].Columns.Add("DateTimeAck");
            }/*
            ds.Tables[0].Columns[4].ColumnName = "DateTimeAck";
            ds.Tables[0].Columns[4].Caption = "DateTimeAck";
            */
            foreach (DataRow rowItem in ds.Tables[0].Rows)
            {
                rowItem["DateTimeSent"] = Convert.ToDateTime(rowItem["DateTimeSent"]).ToString();
                rowItem["DateTimeAck"] = rowItem["DateTimeAck"]+""==""?"":Convert.ToDateTime(rowItem["DateTimeAck"]).ToString();
        ...
        protected void cmdToExcel_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=SentCmds.xls");
            Response.Charset = "";
