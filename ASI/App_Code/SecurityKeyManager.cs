...
namespace VLF.ASI
{
    ...
    public class SecurityKeyManager //: ASLBase
    {
        ...
        public ValidationResult ValidatePasskey(int userId, string passKey)
        {
            ...
            ////    string to = "mvaksman@bsmwireless.com;mmotoarga@bsmwireless.com;";
            ////    string from = "asi@bsmwireless.com";

            ////    try
            ////    {
            ////        MailMessage message = new MailMessage(from, to);
            ////        message.Subject = "ATTENTION: Check IIS on server:" + ConfigurationSettings.AppSettings["ServerIp"].ToString();
            ////        SendEmail(message);
            ////    }
            ////    catch
            ////    {
            ////        Trace.WriteLineIf(AppConfig.tsWeb.Enabled | AppConfig.tsMain.TraceVerbose, CLS.Util.TraceFormat(CLS.Def.Enums.TraceSeverity.WebInterfaces, string.Format("ValidatePasskey: Failed to send IIS reset notification")));
            ////    }
