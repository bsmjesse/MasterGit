...
namespace VLF.ASI.Interfaces
{
   ...
   public class Reports : System.Web.Services.WebService
   {
      ...
      [WebMethod]
      public int GetXml_NewTZ(int userId, string SID, ReportTemplate.ReportTypes repType, string xmlParams, ref string xml,
                     ...
                     ref bool outMaxOverflowed, ref int outMaxRecords)
      {
          try
          {
              ...
              {
                  xml = dbReport.GetXml_NewTZ(repType, xmlParams, userId, "en", ref requestOverflowed, ref totalSqlRecords, ref outMaxOverflowed, ref outMaxRecords);
...
//               if (lang == "fr" && lang != null)
      ...
      // Changes for TimeZone Feature start
      [WebMethod]
      public int GetTripsSummaryDataByLicensePlate_NewTZ(int userId, string SID, string licensePlate,
                                         Int16 sensorNum, DateTime dtFrom, DateTime dtTo, ref string xml)
      {
          try
          {
              ...
              {
                  ds = rpt.GetTripsSummaryReport_NewTZ(userId, licensePlate, sensorNum, dtFrom, dtTo);
...
*/
      ...
      public int DeleteScheduledReportByReportID(int userid, string sid, int reportid)
      {
          try
          {
              ...
              using (ReportScheduler scheduler = new ReportScheduler(LoginManager.GetConnnectionString(userid)))
              {
                  LoggerManager.RecordInitialValues("ReportSchedules", userid, 0, "vlfReportSchedules",
                                                 string.Format("ReportID={0}", reportid.ToString()),
                                                 "Update", this.Context.Request.UserHostAddress,
                                                 this.Context.Request.RawUrl,
                                                 string.Format("Update scheduled report({0}) - Initial values", reportid.ToString()));

                  scheduler.DeleteScheduledReportByReportID(reportid);

                  LoggerManager.RecordUserAction("ReportSchedules", userid, 0, "vlfReportSchedules",
                                                  string.Format("ReportID={0}", reportid.ToString()),
                                                  "Delete", this.Context.Request.UserHostAddress,
                                                  this.Context.Request.RawUrl,
                                                  string.Format("Delete scheduled report({0})", reportid.ToString()));
              }
